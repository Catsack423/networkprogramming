...
3. Generate a Quantum Token:
   - In Terminal 1, type: `/generate "Secret_Meeting"`. 
...
5. Single-Read Limitation:
   - Try to use `/queue` to see tokens bouncing around. Once a token is read, its state is destroyed, proving the No-Cloning theorem.
